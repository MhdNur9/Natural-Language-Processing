...
def delete_letter(word, verbose=False):
    ...
    # delete_l = [l+k[1:] for l, k in split_l]
    for l, k in split_l:
        delete_l.append(l+k[1:])
...
def switch_letter(word, verbose=False):
    ...
    # switch_l = [l + k[1] + k[0] + k[2:] for l, k in split_l if len(k)>1]
    for l, k in split_l:
        if len(k)>1:
            switch_l.append(l + k[1] + k[0] + k[2:])
...
def replace_letter(word, verbose=False):
    ...
    # replace_l = [l+c+k[1:] for l, k in split_l for c in letters]
    for c in letters: 
        for l, k in split_l:
            replace_l.append(l+c+k[1:])
...
def insert_letter(word, verbose=False):
    ...
    # insert_l = [l+c+k for c in letters for l, k in split_l]
    for l, k in split_l:
        for c in letters:
            insert_l.append(l+c+k)
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
   ...
   #Step 1: create suggestions as described above  
    ...
    elif vocab & edit_two_letter:
        suggestions = suggestions or list(vocab & edit_two_letter)
